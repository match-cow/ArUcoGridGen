...
import { Check, ExternalLink, FileDown, Grid2X2, PanelTop, RectangleHorizontal, RectangleVertical, RotateCcw } from "lucide-react";
...
const boardDefaults: Record<BoardType, Board> = {
  aruco: { type: "aruco", dictionary: "DICT_5X5_100", rows: 7, columns: 5, marker_size_mm: 30, separation_mm: 10, show_ids: true, id_font_size_pt: 8 },
...
function NumberField({ label, value, onChange, min, max, step = 1, unit, description, error }: { label: string; value: number; onChange: (n: number) => void; min?: number; max?: number; step?: number; unit?: string; description?: string; error?: string }) {
  ...
  return <label className="field" htmlFor={id}><span className="field-label">{label}{unit && <span aria-hidden>{unit}</span>}</span><div className="input-unit"><Input id={id} type="number" value={Number.isNaN(value) ? "" : value} min={min} max={max} step={step} onChange={(event) => onChange(event.target.valueAsNumber)} aria-invalid={!!error} aria-describedby={error ? `${id}-error` : description ? `${id}-description` : undefined} />{unit && <span aria-hidden>{unit}</span>}</div>{description && <small id={`${id}-description`}>{description}</small>}{error && <small className="field-error" id={`${id}-error`}>{error}</small>}</label>;
}
...
function SwitchField({ label, description, checked, onCheckedChange }: { label: string; description?: string; checked: boolean; onCheckedChange: (value: boolean) => void }) {
  const id = label.toLowerCase().replaceAll(/[^a-z0-9]+/g, "-");
  return <div className="switch-row"><label htmlFor={id}><span>{label}</span>{description && <small>{description}</small>}</label><Switch id={id} checked={checked} onCheckedChange={onCheckedChange} /></div>;
}

function BoardPattern({ type }: { type: BoardType }) {
  if (type === "aruco") {
    const markers = [
      { x: 6, y: 5, cells: [[4, 4], [10, 4], [7, 7], [4, 10]] },
      { x: 29, y: 5, cells: [[4, 4], [7, 4], [10, 7], [7, 10]] },
      { x: 6, y: 28, cells: [[7, 4], [4, 7], [10, 7], [10, 10]] },
      { x: 29, y: 28, cells: [[4, 4], [10, 4], [7, 7], [4, 10], [10, 10]] },
    ];
    return <svg className="board-pattern" viewBox="0 0 56 56" aria-hidden="true"><rect className="pattern-frame" x=".5" y=".5" width="55" height="55" rx="5" />{markers.map((marker, index) => <g key={index} transform={`translate(${marker.x} ${marker.y})`}><rect width="21" height="21" rx="1" /><rect className="pattern-paper" x="3" y="3" width="15" height="15" />{marker.cells.map(([x, y], cell) => <rect key={cell} x={x} y={y} width="3" height="3" />)}</g>)}</svg>;
  }
...
export default function App() {
  ...
  return <TooltipProvider delayDuration={350}><ToastProvider swipeDirection="right">
    <header className="app-header"><div className="brand"><img src="/match.png" alt="MATCH" /><span aria-hidden /><div><h1>Calibration Board Studio</h1><p>Print-ready targets with exact geometry</p></div></div><a className="github-link" href="https://github.com/match-cow/ArUcoGridGen" target="_blank" rel="noreferrer" aria-label="View ArUcoGridGen on GitHub">GitHub<ExternalLink size={14} aria-hidden /></a></header>
    <main className="app-shell">
      <aside className="inspector" aria-label="Board configuration">
        ...
        <Accordion type="multiple" defaultValue={["page", "geometry", "print", "frame"]} className="section-stack">
      ...
      <section className="workspace" aria-live="polite">
        ...
        <div className={`paper-wrap ${status !== "current" ? "muted" : ""}`}>{preview ? <img src={preview} alt="Generated calibration board preview" /> : <div className="skeleton">Generating preview…</div>}{status === "loading" && preview && <div className="overlay">Updating…</div>}</div>
        {status === "validation-error" && <p className="preview-message error">The last valid preview is shown. Adjust the geometry or use Fit to page.</p>}
